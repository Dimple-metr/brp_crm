...
{ 
    //if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) 
	{
		...
		if(strtolower($POST['mode']) == "fetch") {
			$appData = array();
			$i=1;
			$aColumns = array('company_id', 'company_name','comp_add1','comp_email','com_status', 'user_id');
			...
			foreach($sqlReturn as $row) {
				...
				$row_data[] = $row['comp_add1'];
				$row_data[] = $row['comp_email'];
				...
				<button class="btn btn-xs btn-danger" data-original-title="Delete" data-toggle="tooltip" data-placement="top" onClick="delete_data('.$row['company_id'].')"><i class="fa fa-trash-o"></i></button>
			     <a class="btn btn-xs btn-warning" data-original-title="Edit" data-toggle="tooltip" data-placement="top" href="company_pref/'.$row['company_id'].'"> <i class="fa fa-cogs"></i></a>
			';
			...
			echo json_encode( $output );
					...
					$infousr['user_name'] = $infousr['user_company'] =	$info['company_name']= $_POST['company_name'];
					$infousr['user_address'] = $info['address']	=$_POST['address'];
					$infousr['user_mail'] = $_POST['username'];
					$infousr['user_key'] = md5($_POST['password']);
					$infousr['user_type'] = "2";
					$info['bank_name']	= $_POST['bank_name'];
					$info['ac_no']		= $POST['ac_no'];
					$info['ifcs']		= $POST['ifcs'];
					$info['branch_name']= $_POST['branch_name'];
