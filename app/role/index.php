...
if($_POST != NULL) {
	...
	$POST = bulk_filter($dbcon,$_GET);
		...
		if(strtolower($POST['mode']) == "fetch") {
			...
			$appData = array();
			$i=1;
			$aColumns = array('role_id', 'role_name','status','cdate','user_id','deletable');
			$sIndexColumn = "role_id";
			$isWhere = array("status = 0","user_id in (0,$_SESSION[user_id])");
			$sTable = "tbl_role";			
			$isJOIN = array();
			$hOrder = "role_id";
			...
			foreach($sqlReturn as $row) {
				$row_data = array();
				$row_data[] = $row['sr'];
				$row_data[] = $row['role_name'];
			
				if($edit_btn_per){ 
				$edit_btn=' <a class="btn btn-xs btn-warning" data-original-title="Edit" data-toggle="tooltip" data-placement="top" href="'.ROOT.'roleedit/'.$row['role_id'].'"><i class="fa fa-pencil"></i></a>'; 
				}
				if($delete_btn_per){
					$delete_btn=' <button class="btn btn-xs btn-danger" data-original-title="Delete" data-toggle="tooltip" data-placement="top" onClick="delete_role('.$row['role_id'].')"><i class="fa fa-trash-o"></i></button>'; 
				}
			...
			$infom['role_name']=$POST['role_name'];
			$infom['role_module']=implode(",",$POST['user_module']);
			
			$insertidm=add_record('tbl_role', $infom, $dbcon);
			
			foreach($POST['menu_id'] as $key => $name) 
			{	
				$info['role_id']= $insertidm;							
				$info['menu_id']	= $POST['menu_id'][$key];							
				$info['permission']	= '1';
				
				if(in_array($POST['menu_id'][$key],$POST['add_per']))
				{	$info['add_permission']	= '1';}
				else { $info['add_permission']	= '0'; }
				
				if(in_array($POST['menu_id'][$key],$POST['edit_per']))
				{	$info['edit_permission']	= '1';}
				else { $info['edit_permission']	= '0'; }
				
				if(in_array($POST['menu_id'][$key],$POST['view_per']))
				{	$info['view_permission']	= '1';}
				else { $info['view_permission']	= '0'; }
				
				if(in_array($POST['menu_id'][$key],$POST['active_per']))
				{	$info['active_permission']	= '1';}
				else { $info['active_permission']	= '0'; }
				
				if(in_array($POST['menu_id'][$key],$POST['inactive_per']))
				{	$info['inactive_permission']	= '1';}
				else { $info['inactive_permission']	= '0'; }
				
				if(in_array($POST['menu_id'][$key],$POST['export_per']))
				{	$info['export_permission']	= '1';}
				else { $info['export_permission']	= '0'; }
				
				$info['cdate']		= date("Y-m-d H:i:s");
				$info['user_id']	= $_SESSION[user_id];
				$inserid=add_record('tbl_permission_role', $info, $dbcon);
			}
			if($insertidm)
				echo "1";
			else
			...
			$q = $dbcon -> query("SELECT * FROM `tbl_permission_role` WHERE `permission_id` = '$POST[id]'  AND `user_id` = '$_SESSION[user_id]'");
			$r = $q->fetch_assoc();
			echo json_encode($r);
		}
			...
			$insertidm=$POST['eid'];
			$infom['role_name']=$POST['role_name'];
			$infom['user_module']=implode(",",$POST['user_module']);
			
			update_record('tbl_role',$infom,"role_id=".$insertidm,$dbcon);
			
			$deleteid=delete_record('tbl_permission_role',"role_id=".$insertidm,$dbcon);
			...
			$info['status']='2';
			$updateid=update_record('tbl_role', $info,"role_id=".$POST['eid'] , $dbcon);
			
			delete_record('tbl_permission_role',"role_id=".$POST['eid'],$dbcon);
			...
			{
				$chk='';
				$querypermission="select permission,delete_permission,edit_permission from tbl_permission_role where role_id=".$POST['id']."  and menu_id=".$rel_menu['menu_id'];
    ...
    height: 25px;' name='menu_id[]' id=".$rel_menu['menu_id']." onClick='submenuactive();' value=".$rel_menu['menu_id']." />
					...
					{
						$chk1='';
						$querypermission1="select permission,delete_permission,edit_permission from tbl_permission_role where role_id=".$POST['id']." and menu_id=".$rel_submenu['menu_id'];
