...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "fetch") {
		$appData = array();
		$i=1;
		$aColumns = array('usertype_id','usertype_name', 'status', 'cdate', 'user_id','deletable');
		...
		foreach($sqlReturn as $row) {
			...
			if($_SESSION['user_type']=='2'){
				if($row['deletable']=='1'){
					...
					$del_btn='<button class="btn btn-xs btn-danger" data-original-title="Delete" data-toggle="tooltip" 	data-placement="top" onClick="delete_state('.$row['usertype_id'].')"><i class="fa fa-trash-o"></i></button>';
				}
				
				$row_data[] = '<button class="btn btn-xs btn-warning" data-original-title="Edit" data-toggle="tooltip" data-placement="top" onClick="edit_test('.$row['usertype_id'].');"><i class="fa fa-pencil"></i></button> '.$del_btn;
			}
			else{
				$row_data[]='';
			}
