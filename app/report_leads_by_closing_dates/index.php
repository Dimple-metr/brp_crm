...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "generate_report_product_service1")
	{
		...
		/* if($sour){
			...
			$va="e.inquiry_status=0 and et.task_status=0 and et.entry_type=1 and e.inquiry_type_id in (".$sour.")";
			//and instr("'.$fis.'",task.assign_user_ids)'
		}else{
			$va="e.inquiry_status=5";
		} */
		if($POST['start_date']){
			$va="e.inquiry_status=0 and et.task_status=0 and et.entry_type=1 and e.closing_date >= '".date('Y-m-d',strtotime($POST['start_date']))."' AND e.closing_date <= '".date('Y-m-d',strtotime($POST['end_date']))."'";
		}else{
			$va="e.inquiry_status=5";
		}
		$appData = array();
		//'mc.mcd_name as sales_stage'
			$i=1;
			$aColumns = array('e.cdate','e.inquiry_name as oppurtunity_name','mc.mcd_name as sales_stage','mc1.mcd_name as inquiry_type','cust.cust_name','us.user_name as lead_owner','op.opp_stage as stage','e.stage_prob as probablity','e.closing_date','e.inquiry_id');
