...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "fetch") {
		...
		$appData = array();
		$i=1;
		$aColumns = array('task.task_id', 'type.mcd_name', 'regrd.task_rel_name', 'task.task_name', 'task.task_due_date', 'usr.user_name', 'prior.task_priority_name', 'task.task_status','inq.inquiry_no','per.c_con_fname', 'per.c_con_lname', 'cust.cust_name', 'task.task_rel_id', 'task.cdate', 'task.task_completion_date');
...
WHERE task.task_status=0 and task.entry_type=1 and task.alert_date_time!="0000-00-00 00:00:00" and task.alert_date_time!="1970-01-01 05:30:00"  and alert_date_time<"'.date('Y-m-d',strtotime($POST['fil_due_date'])).'" '.$whr.' '.$ftp.' order by task.task_priority_id,task.alert_date_time';
		...
		if(mysqli_num_rows($qry_rs)){
			...
			while($rel=mysqli_fetch_assoc($qry_rs)){
				...
				$add_flp_btn='<a href="'.ROOT.'task_flp/'.$rel['task_id'].'" class="btn btn-xs btn-primary" data-original-title="Add Follow-Up" data-toggle="tooltip" data-placement="top"><i class="fa fa-plus"></i></a>';
				...
				if($rel['task_rel_id']=='5'){//Fixed Type Inquiry
					...
					if($_SESSION['user_type']!='8'){
						$add_quot_btn='<a href="'.ROOT.'inq_to_quot/'.$rel['inquiry_id'].'" data-original-title="Create Quotation" data-toggle="tooltip" data-placement="top" class="btn btn-xs btn-success"><i class="fa fa-plus"></i></a>';
					}
					...
					$add_flp_btn='<a href="'.ROOT.'task_add/'.$rel['inquiry_id'].'" class="btn btn-xs btn-primary" data-original-title="Add Follow-Up" data-toggle="tooltip" data-placement="top"><i class="fa fa-plus"></i></a>';
					...
					$add_apt_btn='<a href="'.ROOT.'appointment_add/'.$rel['inquiry_id'].'" class="btn btn-xs btn-info" data-original-title="Add Appointment" data-toggle="tooltip" data-placement="top"><i class="fa fa-users"></i></a>';
		...
		if($POST['log_user_id']){
			...
			if(!empty($POST['c_user_id'])){
				//$ftp=check_crm_find_in_set($dbcon,$POST['c_user_id'],1);
				$fis=check_crm_find_in_set($dbcon,$POST['c_user_id'],1);
				$ftp=' and instr("'.$POST['c_user_id'].'",task.assign_user_ids)';
			}else{
				$user_id=$_SESSION['user_id'];
				//$ftp=check_crm_find_in_set($dbcon,$user_id,1);
				$fis=check_crm_find_in_set($dbcon,$user_id,1);
				$ftp=' and instr("'.$user_id.'",task.assign_user_ids)';
			}
			//and find_in_set('.$user_id.',task.assign_user_ids)
		}
		$appData = array();
