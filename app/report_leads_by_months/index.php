...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "generate_report_product_service1")
	{
		...
		/* if($POST['start_date']){
			$va="e.inquiry_status=0 and et.task_status=0 and et.entry_type=1 and e.inquiry_date >= '".date('Y-m-d',strtotime($start_date))."' AND e.inquiry_date <= '".date('Y-m-d',strtotime($end_date))."'";
		}else{
