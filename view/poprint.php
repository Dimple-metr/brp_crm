<?php 
	...
	$query="select po.*,ven.cust_name,ven.cust_gst from tbl_purchaseorder as po 
		...
		$po_ref_date='';$delivery_date='';
		if($rel['po_ref_date']!="1970-01-01" && $rel['po_ref_date']!="0000-00-00"){
			$po_ref_date=date('d-m-Y',strtotime($rel['po_ref_date']));
		}
	...
	$tax_types_qry="select sum(tax_amount2) as tax2_amt from tbl_purchaseordertrn where purchaseordertrn_status=0 and purchaseorder_id=".$rel['purchaseorder_id'];
...
<input type="hidden" name="typename" id="typename" value="<?=$rel['invoice_type']?>">
		...
		<tr>
			<th colspan="11" style="padding:0px !important;">
				<table style="font-size:12px;border-collapse: collapse;border-top:none;" cellpadding="0" cellspacing="0" width="100%">
						...
						<tr>
							<td rowspan="4" width="50%" style="vertical-align:top;border:1px solid;">
							<b>To, </b><br/>
							<strong><?=$rel['cust_name']?></strong>
							<br/>
							<span style="font-weight:normal;"><?=nl2br($rel['po_ven_address'])?></span>
								<?if(!empty($rel['cust_gst'])){?>
									<br/><strong>GST No. : <?=$rel['cust_gst']?></strong>
								<?}?> 
							</td>
							<td style="vertical-align:top;border:1px solid;border-right:none;">Purchase Order No 
							</td>
							<td style="vertical-align:top;border:1px solid;border-left:none;"> : <strong><?=$rel['purchaseorder_no']?></strong>
							</td>
						</tr> 
						<tr>
							<td style="vertical-align:top;border:1px solid;border-right:none;">Purchase Order Date 
							</td>
							<td style="vertical-align:top;border:1px solid;border-left:none;"> : <strong><?=date('d-m-Y',strtotime($rel['purchaseorder_date']))?></strong>
							</td>
						</tr>
						<tr>
							<td style="vertical-align:top;border:1px solid;border-right:none;">Ref No.
							</td>
							<td style="vertical-align:top;border:1px solid;border-left:none;"> : <strong><?=$rel['po_ref_no']?></strong>
								</td>
						</tr>
						<tr>
							<td style="vertical-align:top;border:1px solid;border-right:none;">Ref Date
							</td>
							<td style="vertical-align:top;border:1px solid;border-left:none;"> : <strong><?=$po_ref_date?></strong>
							</td>
						</tr>
	...
	<tbody style="border: 1px solid;">
		<? 
			$qry="select trn.*,product.product_name,product.product_hsn,unit_name,group_concat(tax.tax_value) as tax_val,group_concat(tax.tax_name) as tax_name FROM `tbl_purchaseordertrn` as trn 
		...
		<?  } ?>
			...
			<tr>
				...
				<? $r=round($total)-$total; ?>
					...
					<?
						$terms_qry="select potrm.*,mst.tc_name from tbl_po_terms_trn as potrm 
							left join tbl_terms_condition as mst on mst.tc_id=potrm.tc_id
							where potrm.po_terms_trn_status=0 and potrm.purchaseorder_id=".$rel['purchaseorder_id']." order by potrm.tc_priority";
						$terms_qry_rs=$dbcon->query($terms_qry);
						while($term_rel=mysqli_fetch_assoc($terms_qry_rs)){
					?>
						<strong><?=$term_rel['tc_name']?> : </strong> <?=(nl2br($term_rel['tc_details']))?><br/>
					<?
						}
					?>
						</td>
