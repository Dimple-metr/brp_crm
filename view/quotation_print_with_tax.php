...
<div>
	...
	<table style="font-size:12px;border-collapse: collapse;width:100%;" cellpadding="3" cellspacing="3">
		...
		<tbody>';
$trn_qry="select trn.*,pro.product_name,pro.product_desc,pro.product_hsn,unit.unit_name from tbl_quotation_trn as trn 
	...
	$html.='<tr style="border:none;border-left:1px solid;border-right:1px solid;">
		...
		<td style="text-align:left;border:1px solid;vertical-align:top;">
			...
			'.nl2br($trn_rel['product_desc']).'
                ...
                <td style="text-align:center;border:1px solid;vertical-align:top;">'.$trn_rel['product_hsn'].'</td>
...
for($j=0; $j<$pr; $j++)
{
    $html.='<tr style="border:none;border-left:1px solid;border-right:1px solid;">
        ...
        $tax_qry = "select trn.*
            ...
            where trn.quot_trn_status=0 and trn.quotation_id=".$rel['quotation_id'];
        $tax_rel = mysqli_query($dbcon,$tax_qry);
        $tax_data = mysqli_fetch_array($tax_rel,MYSQLI_ASSOC);
        //echo '<pre>';        print_r($tax_data); exit;
        if($tax_data){
            
            $html.='<tr>
                    <td colspan="3" rowspan="6" style="text-align:center;border:1px solid;"></td>
                    <td colspan="3" style="text-align:center;border:1px solid;">SUB TOTAL</td>
                    <td style="text-align:center;border:1px solid;">
                            '.$tax_data['product_rate'].'
                    </td>
            </tr>';
            $html.='<tr>
                    <td colspan="3" style="text-align:center;border:1px solid;">Discount</td>
                    <td style="text-align:center;border:1px solid;">
                            '.$tax_data['product_discount'].'
                    </td>
            </tr>';
            $html.='<tr>
                    <td colspan="3" style="text-align:center;border:1px solid;">TOTAL</td>
                    <td style="text-align:center;border:1px solid;">
                            '.$tax_data['product_amount'].'
                    </td>
            </tr>';

            $html.='<tr>
                    <td colspan="3" style="text-align:center;border:1px solid;">'.$tax_data['tax_name1'].'</td>
                    <td style="text-align:center;border:1px solid;">
                            '.$tax_data['tax_amount1'].'
                    </td>
            </tr>';
            $html.='<tr>
                    <td colspan="3" style="text-align:center;border:1px solid;">'.$tax_data['tax_name2'].'</td>
                    <td style="text-align:center;border:1px solid;">
                            '.$tax_data['tax_amount2'].'
                    </td>
            </tr>';
        }
        $html.='<tr>
		<td colspan="3" style="text-align:center;border:1px solid;"><b>Total</b></td>
		<td style="text-align:center;border:1px solid;"><b>
			'.$tax_data['product_total'].'
		</b></td>
	</tr>';
$html.='</tbody></table>
<table style="font-size:12px;border-collapse: collapse;width:100%;" cellpadding="5" cellspacing="5">
