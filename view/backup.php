...
function EXPORT_TABLES($host,$user,$pass,$name,$flag,$dbcon,  $tables=false, $backup_name=false ){
	...
	if($flag==4)//from setting call Remove transaction tables
	{
		...
		fwrite($handle,$content);
		fclose($handle);
		$alltable=array("coro_chequebook","coro_cheques","coro_vouchers","cust_tempdata","login_history","product_tempdata","tbl_banktransaction","tbl_db_backup","tbl_direct_entry","tbl_estimate","tbl_estimatetrn","tbl_exporttemp","tbl_invoice","tbl_invoicetrn","tbl_invoicetrntemp","tbl_passbookentry","tbl_payment_cheque_generate","tbl_pono","tbl_potrancation","tbl_purchaseorder","tbl_purchaseordertrn","tbl_purchaseproduct","tbl_purchasereceipt","tbl_purchasetrntemp","tbl_receipt","todo_mst","tbl_inward","tbl_inwardtrn","tbl_inwardtrntemp","tbl_per_invoice","tbl_per_invoicetrn","tbl_per_invoicetrntemp","tbl_potrntemp","tbl_serialtrn");
		for($i=0;$i<sizeof($alltable);$i++)
		{
			$tan_query="TRUNCATE TABLE ".$alltable[$i];
			$rel_tan=$dbcon->query($tan_query);
