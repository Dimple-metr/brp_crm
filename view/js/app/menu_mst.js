...
$("#FormEditMenu").on('submit',function(e) {
	...
	var form_data = {
		...
		menu_module: $("#edit_menu_module").val(),
...
function get_all_module(text_id)
{
	Loading(true);
		$.ajax({
			type: "POST",
			url: root_domain+'app/menumst/',
			data: { mode : "get_module" },
			success: function(response)
			{
...
function edit_test(id)
{
		...
		editReq = $.ajax({
			...
			success: function(response)
			{
				...
				get_all_module('edit_menu_module');
				$("#edit_menu_module").select2("val",obj.menu_module);
