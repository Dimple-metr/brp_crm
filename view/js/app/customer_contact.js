...
$("#cust_add").on('submit',function(e) {
	...
	$.ajax({
		cache:false,
		url: root_domain+'app/customer_contact/',
		type: "POST",
		data: form_data,
		contentType: false,
		processData:false,
		...
		{	
			...
			if(responsevalue.trim() == '1') {
				...
				window.location=root_domain+'customer_contact_list';
			}
...
function load_cust_datatable(){
	...
	datatable = $("#dynamic-table").dataTable({
		...
		"iDisplayLength": 10,
		"sAjaxSource": root_domain+'app/customer_contact/',
...
function add_cust_contact()
{
	...
	$.ajax({
		...
		url: root_domain+'app/customer_contact/',
		type: "POST",
		data: { mode : mode , con_first:con_first, con_last:con_last, com_email:com_email,con_mobile:con_mobile,con_phone:con_phone,con_job:con_job,cust_id:cust_id,edit_id:edit_id },
		success: function(responsevalue)
		{	
			//console.log(response);
			//var data = JSON.parse(response);
